...
class TradingStrategy(Strategy):
    def __init__(self):
        ...
        #                      "MTUM", "SPLV", "SOXX"]
        self.tickers = ["QQQ", "TECL", "TQQQ", "DBC", "UUP", "RSP", "TLT", "IEF", "GLD", "XLK", "SOXX", "IJT"]
    ...
    def run(self, data):
        ...
        if ( (positive_momentum_assets <= 3 and mrktclose < mrktema)  or xlu > xli):
            #log(f"RISK OFF: SHV")
    ...
    def calculate_momentum_scores(self, data):
        ...
        for asset in self.tickers:
            ...
            if sma > 0:  # Avoid division by zero
                momentum_score = ( (((close_data / sma)) -1) *2 + ((close_data - close_prices[-self.STMOM]) / close_prices[-self.STMOM]) )
