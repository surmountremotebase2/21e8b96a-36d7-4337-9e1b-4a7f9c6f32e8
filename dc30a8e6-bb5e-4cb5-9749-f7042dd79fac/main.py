...
class TradingStrategy(Strategy):
    ...
    def run(self, data):
        ...
        if spy_ema7[-1] < upper_band and spy_ema7[-5] >= bb["upper"][-5] and spy_ema7[-1] > middle_band:
        ...
        elif spy_ema7[-1] < lower_band and spy_ema7[-3] <= bb["lower"][-3] and spy_rsi[-1] < 30 and mrktSlope[-1] < 0:
            self.trade = 0
            self.count = 5
        elif spy_ema7[-1] > lower_band and spy_ema7[-3] <= bb["lower"][-3] and (spy_rsi[-3] < 35) and self.count < 1:
            self.trade = 1
        elif spy_ema7[-1] >= upper_band and spy_ema7[-2] >= bb["upper"][-2] and spy_rsi[-1] >= 65 and mrktSlope[-1] > 0 and self.count < 1:
            self.trade = 1
            #log("LONG #2")
            #log(str(spy_rsi[-1]))
            #self.count = 0
